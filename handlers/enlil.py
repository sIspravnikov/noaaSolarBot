...
paths = {
    'frames': f"{os.path.join(os.getcwd(),'data',routerName, 'frames')}",
    'gif': f"{os.path.join(os.getcwd(),'data',routerName, 'gif')}",
    'video': f"{os.path.join(os.getcwd(),'data',routerName, 'mp4')}"
}
...
async def cmd_enlil(message: Message):
